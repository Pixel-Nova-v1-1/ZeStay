...
async function fixAvatars() {
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (!isValid) {
                // Ensure name isn't undefined
                const newAvatar = `https://api.dicebear.com/9.x/avataaars/svg?seed=${encodeURIComponent(userData.name || doc.id)}`;
